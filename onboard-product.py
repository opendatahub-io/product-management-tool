...
PLACEHOLDER_CONTAINERFILE = """# PLACEHOLDER — generated by PMT (aipcc-product-management) during Konflux onboarding.
# This component has no real build content yet. Replace this file with the real
# Containerfile when the component build is ready, and ping the AIPCC Productization
# team so the build can be enabled (drop `disabled: true` from the PMT config).
FROM scratch

# Placeholder arg, read from the build-args file referenced by this pipelinerun.
ARG PLACEHOLDER"""

PLACEHOLDER_ARGFILE = """# PLACEHOLDER — generated by PMT (aipcc-product-management) during Konflux onboarding.
# This component has no real build content yet. Replace this file with the real
# build args when the component build is ready, and ping the AIPCC Productization
# team so the build can be enabled (drop `disabled: true` from the PMT config).
...
def render_pipelinerun_templates(
    ...
    for definition in data.get("definitions", []):
        ...
        for component in components:
            ...
            try:
                repo_path = resolve_repo_path(component, gitlab_repo_path, repo_overrides)
                tekton_dir = os.path.join(repo_path, ".tekton")
            except ValueError as e:
            ...
            if tekton_dir not in readme_written:
                readme_written.add(tekton_dir)
                write_with_newline(os.path.join(tekton_dir, "README.md"), _TEKTON_README)
            ...
            for pipelinerun_config in component.get("pipelinerun", []):
                ...
                if pipeline == "disk-image":
                    ...
                    if disabled:
                        cfile_rel = prefix_repo_path(
                            ctx_prefix, component.get("dockerfile", "Containerfile")
                        )
                        cfile_path = os.path.join(repo_path, cfile_rel)
                        if not os.path.exists(cfile_path):
                            ensure_dirs(os.path.dirname(cfile_path))
                            write_with_newline(cfile_path, PLACEHOLDER_CONTAINERFILE)
                            print(
