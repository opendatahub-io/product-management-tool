...
def load_config_and_data(config_file_path):
    ...
    config = {
        ...
        'pipelinerun_template_dir': os.environ.get("PIPELINERUN_TEMPLATE_DIR", "templates/pipelinerun"),
        ...
        for component in definition.get("components", []):
            ...
            for pipelinerun_config in component.get("pipelinerun", []):
                ...
                for pr_type in ["pull", "push"]:
                    ...
                    print(f"Generated {pr_type} pipelinerun for '{component_name}' at {pr_filepath}")
    ...
    for definition in data.get("definitions", []):
        ...
        for component in definition.get("components", []):
            ...
            comp_content = comp_template.render(
                application_name=versioned_app_name,
                component_name=component_name,
                component_context=component.get(
                    "context", component.get("contenxt", ".")
                ),
                component_dockerfile=component.get("dockerfile", "Containerfile"),
                component_url=component["url"],
            ...
            write_with_newline(
                os.path.join(components_dir, comp_filename), comp_content
            )
            ...
            write_with_newline(
                os.path.join(imagerepos_dir, comp_filename), imagerepo_content
            )
        ...
        app_version_directories = ["applications", "components", "imagerepositories", "releaseplans"]
...
def main():
    ...
    if args.mode in ["krd", "both"]:
        ...
        render_krd_templates(data, config['krd_template_dir'], config['krd_path'], config['cluster'])

    if args.mode in ["pipelinerun", "both"]:
        print(f"Generating pipelinerun templates...")
        render_pipelinerun_templates(data, config['pipelinerun_template_dir'], config['gitlab_repo_path'])
